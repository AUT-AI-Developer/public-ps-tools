...
.DESCRIPTION
Runs DISM CheckHealth, conditionally runs DISM ScanHealth and RestoreHealth, and runs SFC verification. The script can download child scripts from GitHub into a local working folder, then calls those scripts out-of-process and returns one consolidated structured result.

The workflow supports Windows client and Windows Server operating systems. It requires administrative elevation and a 64-bit PowerShell process for DISM and SFC work. If launched from a 32-bit PowerShell process on a 64-bit operating system, it relaunches the exact script content already executing through Sysnative Windows PowerShell rather than downloading a second copy.
#>
[CmdletBinding()]
Param(
...
if ([Environment]::Is64BitOperatingSystem -and -not [Environment]::Is64BitProcess) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($PSCommandPath)) {
        ...
        if (-not (Test-Path -LiteralPath $relayDirectory -PathType Container)) { New-Item -Path $relayDirectory -ItemType Directory -Force | Out-Null }
        $relayScriptPath = Join-Path -Path $relayDirectory -ChildPath ('Invoke-WindowsImageHealthRepair-{0}.ps1' -f ([Guid]::NewGuid().ToString('N')))
        $currentScriptText = $MyInvocation.MyCommand.ScriptBlock.ToString()
        Set-Content -LiteralPath $relayScriptPath -Value $currentScriptText -Encoding UTF8
        $deleteRelayScript = $true
    }

    try {
        $scriptCommand = ('& {0} {1}' -f (ConvertTo-SingleQuotedArgument -Value $relayScriptPath), $relayParameters)
        $process = Start-Process -FilePath $sysnativePowerShell -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-Command',$scriptCommand) -Wait -PassThru -WindowStyle Hidden
        exit $process.ExitCode
    }
    finally {
        if ($deleteRelayScript -and $relayScriptPath -and (Test-Path -LiteralPath $relayScriptPath -PathType Leaf)) { Remove-Item -LiteralPath $relayScriptPath -Force -ErrorAction SilentlyContinue }
    }
}
...
function Write-Log {
    ...
    Add-Content -LiteralPath $script:LogPath -Value ('{0} [{1}] {2}' -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'), $Level, $Message) -Encoding UTF8
}
...
function Invoke-ChildHealthScript {
    ...
    $argumentList = @('-NoProfile','-ExecutionPolicy','Bypass','-File',$ScriptPath,'-OutputFormat','None','-ResultPath',$ChildResultPath)
...
$result = [pscustomobject]@{
    ...
    Data = [pscustomobject]@{ Operations = @(); FinalRecommendedAction = 'ReviewLog'; RebootRecommended = $false; WorkingDirectory = $WorkingDirectory; ChildScriptBaseUrl = $ChildScriptBaseUrl; DownloadedScripts = @(); OperatingSystem = $null; Is64BitProcess = [Environment]::Is64BitProcess; PinnedSourceRequired = [bool]$RequirePinnedSource }
    Errors = @()
}
...
foreach ($childScriptName in $childScriptNames) {
    ...
    if (-not $downloaded) {
        $result.Status = 'DependencyMissing'
        $result.ExitCode = 3
        $result.Message = ('Unable to download required child script {0}.' -f $childScriptName)
