...
.PARAMETER LogPath
Optional log path. Defaults to C:\Temp\Invoke-DismScanHealth-<Timestamp>.log.
...
$result = [pscustomobject]@{
    ...
    ExitCode = 1
    Changed = $false
    Message = 'DISM ScanHealth did not complete.'
    LogPath = $LogPath
...
$output = & $dismPath /Online /Cleanup-Image /ScanHealth 2>&1
...
elseif ($outputText -match 'The component store cannot be repaired') {
    ...
    $result.Status = 'Failed'; $result.ExitCode = 1; $result.Message = 'Unable to determine DISM ScanHealth status from output.'; $result.RecommendedAction = 'ReviewLog'; $result.Errors += New-ErrorObject -Message $result.Message -RecommendedAction 'ReviewLog'
}
